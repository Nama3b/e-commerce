...
class ShippingDataTable extends DataTable
{
    ...
    public function dataTable(mixed $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn(__('generate.translate.button.action'), function (Shipping $shipping) {
                return $this->buildAction($shipping);
            })
            ->addColumn('checkbox', function (Shipping $shipping) {
    ...
    {
        foreach (__('generate.shipping.filter') as $key => $value) {
     ...
     * .
     */
    private function buildAction($shipping): string
    {
        $action = Gate::allows(Shipping::EDIT) ? '<a href="javascript:void(0);" class="btn btn-light btn-xs d-inline-flex py-1 mx-1 inline_edit" data-id="' . $shipping->id . '">'
            ...
            . '</svg>'
            . '</a>' : '';
        $action .= Gate::allows(Shipping::DELETE) ? '<a href="javascript:void(0);" class="btn btn-light btn-xs d-inline-flex py-1 mx-1 inline_delete" data-id="' . $shipping->id . '" >'
            . '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">'
     ...
     * @return string
     */
    private function checkbox($shipping): string
    {
        return '<div class="custom-control custom-checkbox">'
            . '<input id="checkRow-' . $shipping->id . '" class="custom-control-input" type="checkbox" value="' . $shipping->id . '" />'
            . '<label for="checkRow-' . $shipping->id . '" class="custom-control-label"></label>'
            . '</div>';
    }
    ...
    public function html(): Builder
    {
        return $this->builder()
            ...
            ->ajax($this->buildAjaxData(Shipping::class, 'shipping'))
            ->orderBy(1)
            ->buttons(['remove'])
