...
class CommentDataTable extends DataTable
{
    ...
    public function dataTable(mixed $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn(__('generate.translate.button.action'), function (Comment $comment) {
                return $this->buildAction($comment);
            })
            ->addColumn('checkbox', function (Comment $comment) {
                return $this->checkbox($comment);
            })
//            ->editColumn('reference_id', function (Comment $comment) {
//                return optional($comment->product)->name;
//            })
            ->editColumn('customer_id', function (Comment $comment) {
                return optional($comment->customer)->full_name;
            })
            ->editColumn('comment_type', function (Comment $comment) {
                return match ($comment->comment_type) {
                    'PRODUCT' => 'Product',
                    'POST' => 'Post',
                };
            })
            ->editColumn('content', function (Comment $comment) {
                return Str::words(strip_tags($comment->content), 10);
            })
    ...
    {
        foreach (__('generate.Comment.filter') as $key => $value) {
     ...
     * .
     */
    private function buildAction($comment): string
    {
        $action = Gate::allows(Comment::EDIT) ? '<a href="javascript:void(0);" class="btn btn-light btn-xs d-inline-flex py-1 mx-1 inline_edit" data-id="' . $comment->id . '">'
            ...
            . '</svg>'
            . '</a>' : '';
        $action .= Gate::allows(Comment::DELETE) ? '<a href="javascript:void(0);" class="btn btn-light btn-xs d-inline-flex py-1 mx-1 inline_delete" data-id="' . $comment->id . '" >'
            . '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">'
     ...
     * @return string
     */
    private function checkbox($comment): string
    {
        return '<div class="custom-control custom-checkbox">'
            . '<input id="checkRow-' . $comment->id . '" class="custom-control-input" type="checkbox" value="' . $comment->id . '" />'
            . '<label for="checkRow-' . $comment->id . '" class="custom-control-label"></label>'
            . '</div>';
    }
    ...
    public function html(): Builder
    {
        return $this->builder()
            ...
            ->ajax($this->buildAjaxData(Comment::class, 'comment'))
            ->orderBy(1)
            ->buttons(['remove'])
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('reference_id')->title('Name/Title')->orderable(false),
            Column::make('customer_id')->title('Customer')->orderable(false),
            Column::make('content')->title('Content')->orderable(false),
            Column::make('comment_type')->title('Type')->orderable(false),
